...
ploss = False # for parametric regularization
pwloss = False # for weight of loss
my_vis = False # False when train (this variable is created by TON)
...
class TextSR(base.TextBase):
    ...
    def train(self):
        ...
        Sigmoid = nn.Sigmoid()
        if ploss:
            Lambda = torch.tensor([-20.], requires_grad=True, device='cuda')
            Degree = torch.tensor([1.5], requires_grad=True, device='cuda')
            pvariable += [Lambda, Degree]
        if pwloss:
            pW = torch.tensor([1., 1.], requires_grad=True, device='cuda')
            pvariable += [pW]
        if ploss or pwloss:
            optimizer_PLoss = torch.optim.SGD(pvariable, lr=1e-3)
                        ...
                        for i in range(self.args.stu_iter):
                            ...
                            if iters % 5 == 0 and i == self.args.stu_iter - 1:
                                ...
                                self.results_recorder.add_scalar('loss/SSIM', float(loss_ssim) * 100,
                                                                 global_step=iters)
                        if pwloss:
                            loss_im = Sigmoid(pW[0]) * loss_img + Sigmoid(pW[1]) * loss_recog_distill
                        else:
                            loss_im = loss_img + loss_recog_distill

                        if ploss:  # Regulalization
                            optim_params = list(aster_student[0].parameters()) + list(model_list[0].parameters())
                            Lambda_ = Sigmoid(Lambda)
                            for ii in range(len(Lambda_)):
                                loss_im += Lambda_[ii] * sum(pp.abs().pow(Degree[ii]).sum() for pp in optim_params)
                    ...
                    if iters % cfg.displayInterval == 0:
                        ...
                        if ploss:
                            ...
                            for ii in range(len(Lambda)):
                                str_ploss += f' [{ii}] lambda = {Lambda[ii]}, degree = {Degree[ii]}'
                            print(str_ploss)
                        if pwloss:
                            print(f'pW0 = {pW[0]}, pW1 = {pW[1]}')
        ...
        for i, data in (enumerate(val_loader)):
            ...
            for batch_i in range(images_lr.shape[0]):
                ...
                if vis or my_vis:
                    ...
                    if self.args.arch in ABLATION_SET and not my_vis:
                        all_label_vecs = {
